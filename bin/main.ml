...
let interpret (in_ : in_channel) =
  ...
  Implicit.Interpreter.eval prog

(* Driver *)
(* TODO: Implement sys-arg-based cmdline interpreter *)
let _ =
  let result =
    In_channel.with_open_text "supplemental/implicit-parameters/samples/sample.ip" interpret
  in
  print_endline (Implicit.Interpreter.res_to_string result)
