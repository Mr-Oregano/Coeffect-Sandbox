...
let rec res_to_string (r : res) = match r with None -> "<NO-OP>" | Some v -> value_to_string v

and value_to_string (v : value) =
  match v with
  | I_Num n -> Int.to_string n
  | I_Clo (v, e, c) -> Printf.sprintf "(%s) |- \\%s. %s" (ctx_to_string c) v (exp_to_string e)

and ctx_to_string (c : ctx) =
  String.concat "; "
    (List.map (fun (name, value) -> Printf.sprintf "%s: %s" name (value_to_string value)) c)

let rec eval (p : prog) : res = match p with None -> None | Some exp -> Some (eval_expr exp [])
